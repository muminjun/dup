...
class ValidatorLocale {
  ...
  static final ValidatorLocale base = ValidatorLocale({
    ...
    ValidationCode.oneOf: (p) =>
        '${p['name']} must be one of: ${p['options']}.',
    ValidationCode.notOneOf: (p) =>
        '${p['name']} must not be one of: ${p['options']}.',
    ValidationCode.condition: (p) =>
        '${p['name']} does not satisfy the condition.',
    ValidationCode.stringMin: (p) =>
        '${p['name']} must be at least ${p['min']} characters.',
    ValidationCode.stringMax: (p) =>
        '${p['name']} must be at most ${p['max']} characters.',
    ValidationCode.matches: (p) =>
        '${p['name']} does not match the required format.',
    ValidationCode.emailInvalid: (p) =>
        '${p['name']} is not a valid email address.',
    ValidationCode.passwordMin: (p) =>
        '${p['name']} must be at least ${p['minLength']} characters.',
    ValidationCode.emoji: (p) => '${p['name']} cannot contain emoji.',
    ValidationCode.mobileInvalid: (p) =>
        '${p['name']} is not a valid mobile number.',
    ValidationCode.phoneInvalid: (p) =>
        '${p['name']} is not a valid phone number.',
    ValidationCode.biznoInvalid: (p) =>
        '${p['name']} is not a valid business registration number.',
    ...
    ValidationCode.alphanumeric: (p) =>
        '${p['name']} must contain only letters and numbers.',
    ValidationCode.numeric: (p) => '${p['name']} must contain only digits.',
    ValidationCode.numberMin: (p) =>
        '${p['name']} must be at least ${p['min']}.',
    ValidationCode.numberMax: (p) =>
        '${p['name']} must be at most ${p['max']}.',
    ...
    ValidationCode.multipleOf: (p) =>
        '${p['name']} must be a multiple of ${p['factor']}.',
    ValidationCode.listNotEmpty: (p) => '${p['name']} cannot be empty.',
    ValidationCode.listEmpty: (p) => '${p['name']} must be empty.',
    ValidationCode.listMin: (p) =>
        '${p['name']} must have at least ${p['min']} items.',
    ValidationCode.listMax: (p) =>
        '${p['name']} cannot have more than ${p['max']} items.',
    ValidationCode.listLengthBetween: (p) =>
        '${p['name']} length must be between ${p['min']} and ${p['max']}.',
    ValidationCode.listExactLength: (p) =>
        '${p['name']} must have exactly ${p['length']} items.',
    ValidationCode.contains: (p) => '${p['name']} must contain ${p['item']}.',
    ValidationCode.doesNotContain: (p) =>
        '${p['name']} must not contain ${p['item']}.',
    ValidationCode.all: (p) =>
        'All items in ${p['name']} must satisfy the condition.',
    ValidationCode.any: (p) =>
        'At least one item in ${p['name']} must satisfy the condition.',
    ValidationCode.none: (p) =>
        '${p['name']} must not contain any items matching the condition.',
    ValidationCode.noDuplicates: (p) =>
        '${p['name']} must not contain duplicate items.',
    ValidationCode.eachItem: (p) =>
        'Item at index ${p['index']} in ${p['name']}: ${p['error']}',
    ValidationCode.dateMin: (p) =>
        '${p['name']} must be on or after ${p['min']}.',
    ValidationCode.dateMax: (p) =>
        '${p['name']} must be on or before ${p['max']}.',
    ValidationCode.dateBefore: (p) =>
        '${p['name']} must be before ${p['target']}.',
    ValidationCode.dateAfter: (p) =>
        '${p['name']} must be after ${p['target']}.',
    ValidationCode.dateBetween: (p) =>
        '${p['name']} must be between ${p['min']} and ${p['max']}.',
    ...
    ValidationCode.startsWith: (p) =>
        '${p['name']} must start with "${p['prefix']}".',
    ValidationCode.endsWith: (p) =>
        '${p['name']} must end with "${p['suffix']}".',
    ValidationCode.stringContains: (p) =>
        '${p['name']} must contain "${p['substring']}".',
    ...
    ValidationCode.creditCard: (p) =>
        '${p['name']} is not a valid credit card number.',
    ValidationCode.koPostalCode: (p) =>
        '${p['name']} is not a valid Korean postal code.',
    // v2 number validators
    ValidationCode.numberPrecision: (p) =>
        '${p['name']} must have at most ${p['digits']} decimal places.',
    ValidationCode.port: (p) =>
        '${p['name']} must be a valid port number (0–65535).',
    // v2 date validators
    ...
    ValidationCode.isSameDay: (p) =>
        '${p['name']} must be the same day as ${p['targetFormatted']}.',
    ValidationCode.isWithin: (p) =>
        '${p['name']} must be within ${p['duration']} of ${p['from']}.',
    // v2 list validators
    ValidationCode.listContainsAll: (p) =>
        '${p['name']} must contain all required items. Missing: ${(p['missing'] as List?)?.join(', ') ?? ''}.',
    // v2 map validators
    ValidationCode.mapMinSize: (p) =>
        '${p['name']} must have at least ${p['min']} entries.',
    ValidationCode.mapMaxSize: (p) =>
        '${p['name']} must have at most ${p['max']} entries.',
    // v2 nested validators
    ValidationCode.nestedFailed: (p) =>
        '${p['name']} has nested validation errors.',
  });
