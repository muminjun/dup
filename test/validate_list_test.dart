...
void main() {
  ...
  group('minLength()', () {
    ...
    test('fails when list has fewer items than min', () {
      final result = ValidateList<int>().setLabel('Tags').minLength(3).validate(
        [1, 2],
      );
      expect(result, isA<ValidationFailure>());
  ...
  group('maxLength()', () {
    ...
    test('fails when list has more items than max', () {
      final result = ValidateList<int>().setLabel('Tags').maxLength(2).validate(
        [1, 2, 3],
      );
      expect(result, isA<ValidationFailure>());
  ...
  group('lengthBetween()', () {
    ...
    test('fails when list has fewer items than min', () {
      ...
      expect(result, isA<ValidationFailure>());
      expect(
        (result as ValidationFailure).code,
  ...
  group('hasLength()', () {
    ...
    test('fails when list is shorter than required', () {
      final result = ValidateList<int>().setLabel('OTP').hasLength(6).validate([
        1,
        2,
      ]);
      expect(result, isA<ValidationFailure>());
  ...
  group('contains()', () {
    ...
    test('fails when item is absent', () {
      final result = ValidateList<int>().setLabel('List').contains(5).validate([
        1,
        2,
        3,
      ]);
  ...
  group('eachItem()', () {
    ...
    test('passes for empty list', () {
      final result = ValidateList<int>().eachItem((_) => null).validate([]);
      expect(result, isA<ValidationSuccess>());
    });
  ...
  group('실제 유저 입력 — 필수 동의 목록 (eachItem)', () {
    ...
    test('3개 모두 동의하면 통과', () {
      expect(
        termsValidator.validate([true, true, true]),
        isA<ValidationSuccess>(),
      );
    });
    ...
    test('첫 번째 항목 미동의 실패 — index 0', () {
      final result =
          termsValidator.validate([false, true, true]) as ValidationFailure;
      expect(result.context['index'], 0);
    });

    test('두 번째 항목 미동의 실패 — index 1', () {
      final result =
          termsValidator.validate([true, false, true]) as ValidationFailure;
  ...
  group('chaining validators', () {
    ...
    test(
      'isNotEmpty + hasNoDuplicates: empty list fails at isNotEmpty first',
      () {
    ...
    test(
      'hasNoDuplicates + eachItem: duplicate detected before individual item check',
      () {
        var eachItemCalled = false;
        final v = ValidateList<int>().hasNoDuplicates().eachItem((item) {
          eachItemCalled = true;
          return null;
        });
  ...
  group('containsAll()', () {
    ...
    test('null passes', () {
      expect(
        ValidateList<int>().containsAll([1]).validate(null),
        isA<ValidationSuccess>(),
      );
    });
    test('empty required list always passes', () {
      expect(
        ValidateList<int>().containsAll([]).validate([]),
        isA<ValidationSuccess>(),
      );
    });
  });
